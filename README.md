# Oscillatory-ML


## Topic 1：Foundations of Synchronization Dynamics

### Main Reference

- **Kuramoto, 1984 — Chemical Oscillations, Waves, and Turbulence** — *Springer Series in Synergetics, Vol. 19 (Book)*, 1984.
- **Winfree, 1967 — Biological Rhythms and the Behavior of Populations of Coupled Oscillators** — *Journal of Theoretical Biology*, 1967.
- **Strogatz, 2000 — From Kuramoto to Crawford: Exploring the Onset of Synchronization in Populations of Coupled Oscillators** — *Physica D: Nonlinear Phenomena*, 2000.

### Related Reference

- Breakspear et al. — *Generative Models of Cortical Oscillations: Neurobiological Implications of the Kuramoto Model* — *Frontiers in Human Neuroscience*, 2010.
- Manoranjani et al. — *Generalization of the Kuramoto Model to the Winfree Model by a Symmetry Breaking Coupling* — *The European Physical Journal Plus*, 2023.

---

## Topic 2：Oscillatory Neural Architectures - I （RNN/GNN）

### Main Reference

- **coRNN: Coupled Oscillatory Recurrent Neural Network** — *ICLR*, 2021.
- **Graph-Coupled Oscillator Networks (GraphCON)** — *ICML*, 2022.

### Related Reference

- Phased LSTM: Accelerating Recurrent Network Training for Long or Event-based Sequences — *NeurIPS*, 2016.
- Oscillations in an Artificial Neural Network Convert Competing Inputs into a Temporal Code — *PLOS Computational Biology*, 2024.
- [From Cortical Synchronous Rhythm to Brain Inspired Learning Mechanism: An Oscillatory Spiking Neural Network with Time-Delayed Coordination](https://arxiv.org/abs/2605.01656) — *arXiv preprint*, 2026.

---

## Topic 3：Oscillatory State-Space Models

### Main Reference

- **[OSCILLATORY STATE-SPACE MODELS]** — ICLR, 2025.
- **[Oscillatory State-Space Models as Inductive Biases for Physics-Informed Neural PDE Solvers](https://arxiv.org/abs/2606.02623)** — *arXiv preprint*, 2026.
- **[Modeling and Stabilizing Perturbation Propagation in Multimodal Learning via Oscillatory State-Space Dynamics](https://doi.org/10.1038/s41598-026-65896-5)** — *Scientific Reports*, 2026.

### Related Reference

- [Learnable Sequential Memory in Coupled Oscillator Networks](https://arxiv.org/abs/2607.18439) — *arXiv preprint*, 2026.

---

## Topic 4：Oscillatory Neural Architectures - II （Oscillatory Neurons）

### Main Reference

- **Artificial Kuramoto Oscillatory Neurons (AKOrN)** — *ICLR*, 2025.
- **[Winfree Oscillatory Neural Network](https://arxiv.org/abs/2605.20922)** — *arXiv preprint*, 2026.
- **KuraNet: Systems of Coupled Oscillators that Learn to Synchronize** — *arXiv preprint*, 2021.
- **[The Kuramoto Neural Operator: Learning to Solve PDEs via Coupled Oscillator Dynamics](https://arxiv.org/abs/2608.10234)** — *arXiv preprint*, 2026.

### Related Reference

- From Coupled Oscillators to Graph Neural Networks: Reducing Over-smoothing via a Kuramoto Model-based Approach — *AISTATS*, 2024.
- Binding Threshold Units with Artificial Oscillatory Neurons — *arXiv preprint*, 2025.
- Kuramoto Oscillatory Phase Encoding: Neuro-inspired Synchronization for Improved Learning Efficiency — *ICML*, 2026.

---

## Topic 5：Attention as Synchronization

### Main Reference

- **[Kuramoto Attention: Synchronizing Self-Attention on the Torus](https://arxiv.org/abs/2606.11585)** — *arXiv preprint*, 2026.
- **[Attention as Frustrated Synchronization](https://arxiv.org/abs/2606.18694)** — *arXiv preprint*, 2026.
- **[Attention by Synchronization in Coupled Oscillator Networks](https://arxiv.org/abs/2606.12059)** — *arXiv preprint*, 2026.
- **Krause Synchronization Transformers** — *ICML*, 2026.

### Related Reference

- The Emergence of Clusters in Self-Attention Dynamics — *NeurIPS*, 2023.
- Hopfield Networks is All You Need — *ICLR*, 2021.
- Energy Transformer — *NeurIPS*, 2023.
- RoFormer: Enhanced Transformer with Rotary Position Embedding — *Neurocomputing*, 2024 (preprint first released in 2021).

---

## Topic 6：Kuramoto for Generative Modeling

### Main Reference

- **Kuramoto Orientation Diffusion Models** — *NeurIPS*, 2025.

### Related Reference

- Score-Based Generative Modeling through Stochastic Differential Equations — *ICLR*, 2021.
- Orientation Diffusions — *IEEE Transactions on Image Processing*, 1998.

---

## Topic 7：AKOrN Applications and Kuramoto Learning / Physical Implementations

### Main Reference

- **Beyond Data Augmentation: Energy-Based Kuramoto Neurons for Semi-Supervised Learning** — *CVPR Workshops*, 2026.
- **[The Phase Is the Gradient: Equilibrium Propagation for Frequency Learning in Kuramoto Networks](https://arxiv.org/abs/2604.10272)** — *arXiv preprint*, 2026.
- **[A Constrained Kuramoto Gradient-Flow System Can Perform High-Accuracy Finite-Time Inference](https://arxiv.org/abs/2609.01539)** — *arXiv preprint*, 2026.
- **[Self-Organized Learning in Oscillatory Neural Networks with Memristive Signed Couplings](https://arxiv.org/abs/2607.00286)** — *arXiv preprint*, 2026.

### Related Reference

- Equilibrium Propagation: Bridging the Gap between Energy-Based Models and Backpropagation — *Frontiers in Computational Neuroscience*, 2017.
- Training and Synchronizing Oscillator Networks with Equilibrium Propagation — *Neuromorphic Computing and Engineering*, 2025.
- Energy Transformer — *NeurIPS*, 2023.
- [Design of Oscillatory Neural Networks Using Machine-Learned Templates](https://doi.org/10.3390/electronics15132897) — *Electronics*, 2026.
- [Graph Coloring Approach to Solving Sudoku with Oscillatory Neural Networks](https://arxiv.org/abs/2607.15814) — *arXiv preprint*, 2026.
- [ODEONN: A Digital ODE Solver Architecture for Oscillatory Neural Networks](https://arxiv.org/abs/2608.20110) — *arXiv preprint*, 2026.
